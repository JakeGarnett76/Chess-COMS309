package com.carverbartz.advancedchess;
...
public class Piece {
     ...
     * @param t- the string representing the type of piece
     * @param playerNum- which player the piece belongs to
     */
    Piece(int x, int y, String t, int playerNum){
        coX = x;
        coY = y;
        player = playerNum;
        type = t;
        ...
        if((xn < 0 || xn > 7) || (yn < 0 || yn > 7))
            ...
            switch (getStr().toUpperCase()) {
                ...
                case ("Q"):
                    ...
                    break;
                case ("K"):
                    if(diffX + diffY == 1 || diffX + diffY == 2)
                        val = true;
                    break;
                case ("R"):
